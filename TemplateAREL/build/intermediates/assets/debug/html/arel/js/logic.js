...
arel.sceneReady(function() {
    ...
    scenario.currentTrackingConfigPathOrIdentifier = "html/resources/TrackingConfig_ab1ab6b565bf56573eb63820371c278c.zip";
    ...
    scenario.onStartup = function () {
        arel.Debug.log("Welcome to the 'prueba' Augmented Reality experience.");
        ...
        scenario.registerObject(model11);
        if (methodExists(model11, model11.onLoaded)) {
            model11.onLoaded();
        }
    ...
    // cubo5obj
    if (!arel.Scene.objectExists("model11")) {
        arel.Debug.log("ERROR: retrieving the object model11, it seems that it does not exist. Maybe it would help cleaning the cache or a problem occurred while reloading the channel.");
    }

    var model11 = arel.Scene.getObject("model11");
    model11.objectName = "model11";
    model11.type = "Model";
    model11.scene = scene1;
    model11.associatedTrackable = pattern2;
    model11.displayOnLoaded = false;
    scenario.contents.push(model11);

    model11.setScene = function (scene) {
        this.scene = scene;
        scenario.currentScene.display();
    };

    model11.isLoaded = function () {
        return arel.Scene.objectExists("model11");
    };

    model11.bind = function (cosID) {
        arel.Debug.log(this.objectName + ".bind(" + cosID + ")");
        this.setCoordinateSystemID(cosID);
    };

    model11.load = function () {
        arel.Debug.log(this.objectName + ".load()");
        if (!this.isLoaded()) {
            scenario.addObject(this);
        }
    ...
    model11.hide = function () {
        arel.Debug.log(this.objectName + ".hide()");
        this.setVisibility(false);
    };

    model11.attach = function (origin, offset) {
        arel.Debug.log(this.objectName + ".attach(" + origin.objectName + ")");
        if (typeof (origin.getScreenAnchor()) != 'undefined' && typeof (origin.getScreenAnchorFlags()) != 'undefined') {
            this.setScreenAnchor(origin.getScreenAnchor(), origin.getScreenAnchorFlags());
        }
        ...
        if (origin.groupID) {
            if (this.groupID) {
                arel.GestureHandler.removeObject(model11);
            }
            arel.GestureHandler.addObject("model11", origin.groupID);
            this.setPickingEnabled(false);
        }
    };

    model11.play = function (animationName, animationLooped) {
        arel.Debug.log(this.objectName + ".play(" + animationName + ", " + animationLooped + ")");
        this.startAnimation(animationName, animationLooped);
        if (methodExists(this, this.onPlayed)) {
            this.onPlayed(animationName);
    ...
    // cubo5obj
    if (!arel.Scene.objectExists("model7")) {
        arel.Debug.log("ERROR: retrieving the object model7, it seems that it does not exist. Maybe it would help cleaning the cache or a problem occurred while reloading the channel.");
    }
